...
if(!empty($_POST['nombre']) && !empty($_POST['password'])){ //comprobamos si recibe info y si no están vacíos

    $conexion = conectarse($servidor, $usuariodb, $clavedb, $opcionesDB); //llamamos a la función que conecta con la bbdd
    $sql = 'SELECT * FROM usuarios WHERE nombre = :nombre'; //inicializamos la variable sql
    $stmt = $conexion->prepare($sql); //preparamos la consulta preparada
    $stmt->bindParam(':nombre', $_POST['nombre'], PDO::PARAM_STR); //asignamos el valor del campo nombre a la variable :nombre, indicando que la procese como una cadena de texto
    ...
    if($reg = $stmt->fetch()){ //si el fetch da false, el usuario no existe y no entra. Si es true, si hay datos y se asignan a $reg
        ...
        if(password_verify($_POST['password'], $reg['password'])){ //si la clave es correcta (true), se ha identificado al usuario
            $_SESSION['id_usuario'] = $reg['id_usuario']; //creamos la variable de sesión de usuario que recoge el id del usuario
            $_SESSION['nombre'] = $reg['nombre']; //creamos la variable de sesión de usuario que recoge el nombre del usuario
            $patras = false; //indicamos que no debe volver al index.php pq se ha identificado correctamente
            header('Location: compra.php'); //redireccionamos al usuario a la página de compra
